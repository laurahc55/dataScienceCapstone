...
def get_scatter_plot(entered_site, entered_payload):
    ...
    if entered_site == 'ALL':
        spacex_df2 = spacex_df[(spacex_df['Payload Mass (kg)'] >= entered_payload[0]) & (spacex_df['Payload Mass (kg)'] <= entered_payload[1])]
        fig2 = px.scatter(spacex_df2, y='class', x='Payload Mass (kg)',
        color="Booster Version Category",
        title='Correlation between Payload and Success for all Sites')
        return fig2
    else:
        fig2 = px.scatter(filtered_df, y='class', x='Payload Mass (kg)', 
